...
public class Character extends Unit implements GameAction {
    ...
    // ultra : 특수 기술에 대한 공격력
    // hp : 체력
    // 캐릭터가 가지는 기본적인 능력치를 생성자를 통해서 결정 (캐릭터간의 밸런스를 맞춤)
    public Character(String name, int pP, int mP, double pH, double mH, int ultra, int hp) {

        this.name = name; // this 는 자신의 객체를 참조하는 참조 변수
        this.pPower = pP;
        this.mPower = mP;
        this.pHit = pH;
        this.mHit = mH;
    ...
    public boolean setDamage(double damage) {
        ...
        if(hp > damage) {
            ...
            return false; // false 는 캐릭터가 살아 있음을 의미

        } else {
            System.out.println(name + "가 죽었습니다. 게임을 종료합니다.");
            return true;
