...
public class GameThB extends Thread {
    ...
    public GameThB(Character warrior, Character wizard) {
        ...
        this.warrior = warrior; // 매개변수로 넘겨 받은 객체를 인스턴스 필드 변수에 넘겨줌
        this.wizard = wizard;
    }
    ...
    public void run() {
        ...
        while(true) { // 해당 쓰레드를 무한 반복하도록 만듦
            ...
            try {
                ...
                if(normal == 1) {
                    System.out.println("마법 공격 : " + wizard.name + "가 " + warrior.name + "에게 " +
                            wizard.mAttack() + "의 데미지를 입혔습니다.");
                    endGame  = warrior.setDamage(wizard.mAttack());

                } else {
                    System.out.println("물리 공격 : " + wizard.name + "가 " + warrior.name + "에게 " +
                            wizard.pAttack() + "의 데미지를 입혔습니다.");
                    endGame  = warrior.setDamage(wizard.pAttack());
                }

                if(special == 10) { // 1/18 확률로 발동
                    System.out.println(wizard.name + " 궁극기 발동 !!!!, " + wizard.name + "가 "
